  ...
  public List<Article> findAll() {
    return jdbi.inTransaction((Handle handle) -> {
      List<Map<String, Object>> result =
              handle.createQuery("SELECT id, name, trending FROM article")
                      .mapToMap()
                      .list();
      return result.stream().map(element -> new Article(
              (new ArticleId((Long) element.get("id"))),
              ((String) element.get("name"))
      ).setTrending((boolean) element.get("trending"))).collect(Collectors.toList());
    });
  }
    ...
    return jdbi.inTransaction((Handle handle) -> {
      try {
        Map<String, Object> result =
                handle.createQuery("SELECT id, name, trending FROM article WHERE id = :id")
        ...
        ).setTrending((boolean) result.get("trending"));
      } catch (IllegalStateException e) {
        throw new ArticleNotFoundException("Have no article with this id");
      }
    });
  }
