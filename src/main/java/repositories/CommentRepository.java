  ...
  public List<Comment> findAllCommentsOfUser(ArticleId articleId) {
    return jdbi.inTransaction((Handle handle) -> {
      List<Map<String, Object>> result =
              handle.createQuery("SELECT id, text FROM comment WHERE article_id = :articleId")
    ...
    return jdbi.inTransaction((Handle handle) -> {
      try {
        Map<String, Object> result =
                handle.createQuery("SELECT article_id, text FROM comment WHERE id = :id")
        ...
        throw new CommentNotFoundException("have no comment with this id");
      }
    });
  }
